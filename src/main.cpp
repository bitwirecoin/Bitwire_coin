...
uint256 hashGenesisBlock("0x000004708e4fae899926f3e0cde5d27d26a12bc17022707b255104df6f0a7260");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bitwire: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bitwire Signed Message:\n";
...
{
    // Bitwire: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 3 * 60; // Bitwire: each block
static const int64 nTargetSpacing = 3 * 60; // Bitwire: 300 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bitwire: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, Bitwire - DarkGravity, written by Evan Duffield - evan@Bitwire.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bitwire: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Bitwire: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "BitwireXX";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1528203925;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 443596744;
        ...
        assert(block.hashMerkleRoot == uint256("0x8e2029f9f78efb86002345fd996e29ddf312c2c0c4d83b3b90883e77aaee45e3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // Bitwire: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitwireMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitwireMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitwireMiner(CWallet *pwallet)
{
    printf("BitwireMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Bitwire-miner");
    ...
    try { while (true) {
        ...
        printf("Running BitwireMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitwireMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitwireMiner, pwallet));
}
